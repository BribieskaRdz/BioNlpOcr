...
public class Camera extends Activity implements SurfaceHolder.Callback, View.OnClickListener, Runnable 
{
	/*
	 * ----------------------------------------------------------------------------------------
	 * the CONSTANTS
...
//	private boolean m_bLineMode = false;					// capture-mode: line/all
	...
	private static SlidingDrawer menu_prefs;
	private static TextView idioma;
	private static LinearLayout cambia_idioma;
...
//	private CustomImageButton m_btGotoResults;
	...
	public void onCreate(Bundle savedInstanceState) 
	{
		...
		try
		{
			...
			ImageView shot = (ImageView)findViewById(R.id.imageViewShot);
			...
			menu_prefs = (SlidingDrawer) findViewById(R.id.slidingDrawer1);
			idioma = (TextView) findViewById(R.id.textViewIdioma);


			
			
			final ImageView flash = (ImageView)findViewById(R.id.ImageViewFlash);
...
//						
						Toast.makeText(getApplicationContext(), "Flash", Toast.LENGTH_LONG).show();
						
						
						
						Parameters p = CameraManager.get().GetCameraParameters();
						
						if (p.getFlashMode().equals(Parameters.FLASH_MODE_OFF)) {
							p.setFlashMode(Parameters.FLASH_MODE_TORCH);
							CameraManager.get().SetCameraParameters(p);
							
							flash.setImageDrawable(getResources().getDrawable(R.drawable.device_access_flash_off));
							
						} else {
							p = CameraManager.get().GetCameraParameters();
							p.setFlashMode(Parameters.FLASH_MODE_OFF);
							CameraManager.get().SetCameraParameters(p);
							
							flash.setImageDrawable(getResources().getDrawable(R.drawable.device_access_flash_on));
			...
			bt.SetImage(R.drawable.device_access_camera, 0, 0);
			...
			LinearLayout fondo_slider = (LinearLayout) findViewById(R.id.linearLayoutfondo);
			fondo_slider.getBackground().setAlpha(175);
			
			
			
			
			m_clCapture = (CaptureLayout)findViewById(R.id.mezzofanti_capturelayout_view);			
			m_clCapture.SetLineMode(false);
...
//			m_bSkipIntroAtStartup = prefs.getBoolean(PreferencesActivity.KEY_SKIP_INTRO_AT_STARTUP, true);
			m_bSkipIntroAtStartup = prefs.getBoolean(PreferenciasActivity.KEY_SKIP_INTRO_AT_STARTUP, true);
...
//				ShowLineModeButtons(true);
	...
	protected void onResume() 
	{
		...
		idioma.setText(OCR.mConfig.GetLanguageMore());
		
		cambia_idioma = (LinearLayout) findViewById(R.id.LinearLayoutCambiarIdioma);
		cambia_idioma.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub
				OCR.Initialize();
				if (OCR.mConfig.GetLanguageMore().equals("English")) {
					OCR.get().SetLanguage("spa");
				} else if (OCR.mConfig.GetLanguageMore().equals("Spanish")){
					OCR.get().SetLanguage("eng");
				}
				OCR.Initialize();
				idioma.setText(OCR.mConfig.GetLanguageMore());					
			}
...
//			intent.setClassName(this, PreferenciasActivity.class.getName());
...
//			builderH.setIcon(R.drawable.action_help);
...
//				m_clCapture.ShowWaiting(getString(R.string.mezzofanti_capturelayout_takingpicture));
	...
	public boolean onKeyUp(int keyCode, KeyEvent event) 
	{
	    ...
	    if (keyCode == KeyEvent.KEYCODE_MENU) {
	    	...
	    	if (menu_prefs.isOpened()) {
				menu_prefs.animateClose();
			} else {
				menu_prefs.animateOpen();
			}
...
//			String m_sOCRResult = OCR.get().ImgOCRAndFilter(iImage, iPicWidth, iPicHeight, m_bHorizDispAtPicTaken, m_bLineMode);	
			String m_sOCRResult = OCR.get().ImgOCRAndFilter(iImage, iPicWidth, iPicHeight, m_bHorizDispAtPicTaken, false);	
...
//				}
	...
	{
		AlertDialog.Builder builderH = new AlertDialog.Builder(this);
		builderH.setIcon(R.drawable.alert_dialog_icon);
	...
	private void StartResultsActivity()
	{
		if (m_bSdcardMounted) 
		{
			...
			bun.putString("m_sOCRResultLineMode", m_sOCRResultLineMode);
//			bun.put
//			m_bmOCRBitmap 
//			bun.putBoolean("bLineMode", m_bLineMode);
			bun.putBoolean("bLineMode", false);
			intent.putExtras(bun);
//			intent.setClassName(this, ResultsActivity.class.getName());
			intent.setClassName(this, ResultadoActivity.class.getName());
			startActivity(intent); 
		}		
	}
...
//				ShowOptionsMenu(false);
				...
				{
					UpdateStorageHint(false);
					Toast.makeText(Camera.this, R.string.mezzofanti_sdcard_wait, 5000);
				} 
				else 
